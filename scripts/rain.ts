...
// 绘制代码雨
function rain(canvas: HTMLCanvasElement, onFinish: FinishFn) {
  const ctx = canvas.getContext("2d") as CanvasRenderingContext2D;
  canvas.width = screen.availWidth;
  canvas.height = screen.availHeight;

  // 字母纵坐标
  const ys: number[] = Array.apply(null, Array(Math.ceil(canvas.width / fontSize))).map(() => {
